...
def _res_block(in_tensor, filters, downsample=True, bottle_neck=True):
    res = in_tensor
    strides = 1
    if downsample:
        strides = 2
...
def _convx(in_tensor, filters, n_times, downsample=True, bottle_neck=True):
    res = _res_block(in_tensor, filters, downsample, bottle_neck)
    for i in range(n_times-1):
        res = _res_block(res, filters, bottle_neck=bottle_neck)
    return res

def _resnet(in_shape=(224, 224, 3), n_classes=100, opt='adam', units_by_block=[64, 128, 512, 256], n_convx=[3, 4, 4, 4], bottle_neck=False):
    X_in = Input(in_shape)
    X = _pre_block(X_in)
    resblock_1 = _convx(X, units_by_block[0], n_convx[0], False, bottle_neck)
    resblock_2 = _convx(resblock_1, units_by_block[1], n_convx[1], bottle_neck=bottle_neck)
    resblock_3 = _convx(resblock_2, units_by_block[2], n_convx[2], bottle_neck=bottle_neck)
    resblock_4 = _convx(resblock_3, units_by_block[3], n_convx[3], bottle_neck=bottle_neck)
    Y_out = _post_block(resblock_4, n_classes)
    model = Model(X_in, Y_out)
    return model

def resnet(in_shape=(224, 224, 3), n_classes=100, opt='adam', units_by_block=[64, 128, 512, 256], n_convx=[3, 4, 4, 4], bottle_neck=False):
    return _resnet(in_shape, n_classes, opt, units_by_block, n_convx, bottle_neck)
